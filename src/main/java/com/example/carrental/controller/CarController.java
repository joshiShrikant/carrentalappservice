...
public class CarController {
    ...
    public ResponseEntity<CarDto> getCar(@PathVariable Long id) {
        return carRepository.findById(id).map(car -> ResponseEntity.ok(CarMapper.toDto(car))).orElse(ResponseEntity.notFound().build());
    }

    @GetMapping
    @Operation(summary = "get Cars", description = "get all Cars")
    public List<CarDto> getCars() {
        return carRepository.findAll().stream().map(CarMapper::toDto).collect(Collectors.toList());
    }

    @PostMapping
    @Operation(summary = "add Car", description = "add a Car")
    ...
    @PatchMapping("/{id}")
    @Operation(summary = "patch Car", description = "patch a Car by id")
